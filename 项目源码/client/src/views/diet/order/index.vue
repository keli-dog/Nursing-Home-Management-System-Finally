...
import { addOrder, pageOrderByKey, sendOrder } from "@/apis/order";
...
const getTableList = (params: any) => {
  ...
  return pageOrderByKey(newParams);
};
...
const openDrawer = (title: string, rowData: any = {}) => {
  const params = {
    ...
    rowData: { ...rowData.row },
    isView: title === "查看",
    api: title === "新增" ? addOrder : title === "送餐" ? sendOrder : "",
...
const columns: ColumnProps<any>[] = [
  ...
  { prop: "dineDate", label: "就餐时间" },
  { prop: "dineType", label: "就餐方式" },
  { prop: "staffName", label: "送餐人员姓名" },
  { prop: "deliverDishesDate", label: "送餐时间" },
