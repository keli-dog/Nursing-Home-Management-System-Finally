...
import accidentDialog from "./accidentDialog/index.vue";
import { Plus } from "@icon-park/vue-next";
import { ColumnProps } from "@/components/ProTable/interface";
import ProTable from "@/components/ProTable/index.vue";
import { Delete, EditPen, View } from "@element-plus/icons-vue";
import { addAccident, deleteAccident, editAccident, pageAccidentByKey } from "@/apis/accident";
...
const getTableList = (params: any) => {
  ...
  return pageAccidentByKey(newParams);
};

// 删除
const deleteData = async (params: any) => {
  const res: any = await deleteAccident({
    accidentId: params.row.id
  });
  if (res.code === 200) {
...
const openDrawer = (title: string, rowData: any = {}) => {
  const params = {
    ...
    rowData: { ...rowData.row },
    isView: title === "查看",
    api: title === "新增" ? addAccident : title === "编辑" ? editAccident : "",
...
const columns: ColumnProps<any>[] = [
  ...
  { prop: "staffName", label: "护工姓名", search: { el: "input" } },
  { prop: "occurDate", label: "发生时间" },
