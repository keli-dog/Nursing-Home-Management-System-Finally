  ...
  <div class="table-box">
    ...
    >
      ...
      <template #tableHeader>
        <el-button @click="openElderDrawer">
...
import { addRetreatApply, pageRetreatApplyByKey, pageSearchElderByKey } from "@/apis/retreatApply";
...
const getTableList = (params: any) => {
  ...
  return pageRetreatApplyByKey(newParams);
};
...
// 获取选择老人的信息
const getCheckElderInfo = async (row: any) => {
  const res: any = await addRetreatApply({
    elderId: row.id
  });
  if (res.code === 200) {
...
const columns: ColumnProps<any>[] = [
  ...
  { prop: "elderName", label: "姓名", search: { el: "input" } },
  { prop: "elderSex", label: "性别", search: { el: "input" } },
  { prop: "idNum", label: "身份证号", search: { el: "input" } },
  { prop: "bedName", label: "床位名称", search: { el: "input" } },
  { prop: "applyFlag", label: "审核状态" }
];
</script>
