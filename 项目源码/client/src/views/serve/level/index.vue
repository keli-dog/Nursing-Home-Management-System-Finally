...
import levelDialog from "./levelDialog/index.vue";
...
const getTableList = (params: any) => {
  ...
  return pageNurseGradeByKey(newParams);
};

// 删除
const deleteData = async (params: any) => {
  const res: any = await deleteNurseGrade({
    nurseGradeId: params.row.id
  });
  if (res.code === 200) {
...
const openDrawer = (title: string, rowData: any = {}) => {
  const params = {
    ...
    rowData: { ...rowData.row },
    isView: title === "查看",
    api: title === "新增" ? addNurseGrade : title === "编辑" ? editNurseGrade : "",
...
const columns: ColumnProps<any>[] = [
  ...
  { prop: "name", label: "护理等级名称", search: { el: "input" } },
  {
    enum: nurseTypeList,
    prop: "type",
    label: "护理类型",
