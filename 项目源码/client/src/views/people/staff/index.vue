...
import staffDialog from "./staffDialog/index.vue";
...
import { addStaff, editStaff, getRole, leaveStaff, pageStaffByKey } from "@/apis/staff";
...
const getTableList = (params: any) => {
  ...
  return pageStaffByKey(newParams);
};

// 删除
const deleteData = async (params: any) => {
  const res: any = await leaveStaff({
...
const openDrawer = (title: string, rowData: any = {}) => {
  const params = {
    ...
    rowData: { ...rowData.row },
    isView: title === "查看",
    api: title === "新增" ? addStaff : title === "编辑" ? editStaff : "",
...
const columns: ColumnProps<any>[] = [
  ...
  { prop: "name", label: "员工姓名", width: 100, search: { el: "input" } },
  {
    enum: getRole,
    prop: "roleName",
    label: "角色名称",
  ...
  { prop: "sex", label: "员工性别", width: 100 },
  { prop: "phone", label: "员工电话", search: { el: "input" } },
  { prop: "email", label: "员工邮箱" },
